...
@property (weak, nonatomic) IBOutlet UIButton *reloadeBtn;
...
//    if (self) {
//        self = [[[NSBundle mainBundle] loadNibNamed:@"VideoPlayer" owner:self options:nil] firstObject];
...
// 初始化函数
+ (VideoPlayer *) initVideoPlayerWithSupeView:(UIView *) superView videoImage:(UIImage *)image
{
    VideoPlayer *videoView  = [[[NSBundle mainBundle] loadNibNamed:@"VideoPlayer" owner:self options:nil] firstObject];
    videoView.frame = CGRectMake(0, 0, superView.frame.size.width, superView.frame.size.height);
    //    [self addSubview:self.view];
    [videoView.playProgress setThumbImage:[UIImage imageNamed:@"MoviePlayer_Slider"] forState:UIControlStateNormal];
    [videoView setPortraitLayout];
    videoView.player = [[AVPlayer alloc] init];
    videoView->_playerLayer = [AVPlayerLayer playerLayerWithPlayer:videoView.player];
    [videoView.playerView.layer addSublayer:videoView->_playerLayer];
    videoView.videoImage.image = image;
    return videoView;
}
...
//    // 加载失败通知
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(playToEndTimeFaile:) name:AVPlayerItemFailedToPlayToEndTimeNotification object:nil];
//   [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(playToEndTimeFaile1:) name:AVPlayerItemFailedToPlayToEndTimeErrorKey   object:nil];
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(playToEndTimeFaile2::) name:AVPlayerItemNewErrorLogEntryNotification   object:nil];
    // 后台&前台通知
...
-(void)playbackFinished:(NSNotification *)notification{
    ...
    [self insertSubview:self.view belowSubview:self.videoImage];
}
//- (void)playToEndTimeFaile:(NSNotification *)notification
//{
//    NSLog(@"播放失败");
//    _playerItem = [notification object];
//    self.reloadeBtn.hidden = NO;
//}
//- (void)playToEndTimeFaile1:(NSNotification *)notification
//{
//    NSLog(@"播放失败1");
//    _playerItem = [notification object];
//    self.reloadeBtn.hidden = NO;
//}
//- (void)playToEndTimeFaile2:(NSNotification *)notification
//{
//    NSLog(@"播放失败2");
//    _playerItem = [notification object];
//    self.reloadeBtn.hidden = NO;
...
// 重载
- (IBAction)reloade:(id)sender {
    _playerItem = nil;
    [self updatePlayerWith:self.videoURL];
    self.reloadeBtn.hidden = YES;
